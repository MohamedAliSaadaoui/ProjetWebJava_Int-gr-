...
public function newUser(Request $request, UserPasswordHasherInterface $passwordHasher, EntityManagerInterface $entityManager): Response
{
    ...
    if ($form->isSubmitted()) {
        if ($form->isValid()) {
            try {
                // Récupérer le mot de passe en clair seulement si un nouveau mot de passe est fourni
                ...
                if ($plainPassword) {
                    ...
                    // Si aucun mot de passe n'est fourni (cas d'une modification), ne rien changer
...
public function resetPassword(Request $request, MailerInterface $mailer, EntityManagerInterface $entityManager): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        if ($user) {
            ...
            // Ajouter un message d'erreur si l'email n'est pas trouvé
